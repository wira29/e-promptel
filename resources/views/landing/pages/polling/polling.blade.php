...
<div class="bg-white pt-10 pb-50">
    <!-- Modal -->
    <div class="modal fade" id="modal-respondent" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h1 class="modal-title fs-5" id="exampleModalLabel">Isi Polling</h1>
                ...
                <div class="modal-body">
                    <form id="form-respondent" action="" method="POST">
                        ...
                        <div>
                            ...
                            <input type="text" name="name" class="form-control mt-10" placeholder="Nama">
                        </div>
                        <div class="mt-20">
                            <label for="">Email</label>
                            <input type="email" name="email" class="form-control mt-10" placeholder="Email">
                        </div>
                    </form>
                </div>
                <div class="modal-footer">
                    <button id="btn-submit" type="button" class="btn btn-primary">Isi Polling</button>
                </div>
            </div>
        </div>
    </div>
                                    ...
                                    <div class="row mb-40 list-style-2">
                                        <div class="col-md-12 align-self-center">
                                            <div class="post-content">
                                                ...
                                                <p>{{ substr(strip_tags($poll->description), 20) }}...</p>
                                                <div
                                                    class="col-12 mb-10 entry-meta meta-1 float-start font-x-small text-uppercase"
                                                >
                                                <span class="post-on"
                                                >{{ \Carbon\Carbon::make($poll->start)->format('d F Y') }} - {{ \Carbon\Carbon::make($poll->end)->format('d F Y') }}</span
                                                >
                                                </div>
                                                <button data-poll="{{ $poll }}" class="btn btn-primary btn-polling" type="button" data-bs-toggle="modal" data-bs-target="#exampleModal">
                                                    Isi Polling
                                                </button>
                                            </div>
                                        </div>
    ...
    <script>
        $(document).ready(function() {
            $('.btn-polling').click(function() {
                ...
                $('#form-respondent').attr('action', `{{ route('landing.polling-form', ':id') }}`.replace(':id', poll.id))
